...
class GoogleClient
{
    ...
    function getRecords($start_search, $end_search, $how_long)
    {
        $client = $this->getClient();
        $service = new Google_Service_Calendar($client);
        ...
        return $this->getFree($start_search, $end_search, $event_list, $how_long);
    }

    private function getFree($start, $end, $events, $how_long)
    {
        ...
        while ($appt_start_time < $end) {
            ...
            foreach ($events as $event) {
                ...
                if (($appt_start_time >= $event_start && $appt_start_time < $event_end) ||
                    ...
                    (strtotime($appt_start_time . '+' . $how_long . ' minutes') < $event_end)) {
            ...
            if ($slot_available) {
                ...
                if ($date->format('H') >= 9 && $date->format('H') <= 19) {
